...
Focused category mode:
  - If additional context contains question_category, conduct a focused diagnostic for that category only.
  - Retrieve records using that selected category, ask confirmed returned records in order, then move to the closing script.
...
If question_category is present in additional context, use only that focused category instead of routing across all states.
