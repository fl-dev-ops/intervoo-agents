...
Standalone LiveKit voice agent for job search coaching with persistent memory. Runs mock interviews for entry-level roles and remembers user context across sessions. Deployed independently with the worker name `job-finder-agent`.
...
- `src/memory.py`: Mem0 helpers for persistent memory across sessions
...
The agent uses a ChromaDB knowledge base to retrieve job-specific questions at runtime. The `retrieve_knowledge` tool fetches relevant records during the conversation based on the chosen category.
...
| `AGENT_NAME` | `job-finder-agent` | LiveKit worker registration name |
...
| `SARVAM_TTS_SPEAKER` | `ritu` | Sarvam TTS speaker voice |
